defmodule BusiApiWeb.BusinessController do
  ...
  def index(conn, _params) do
    businesses = DexContext.list_businesses()
    render(conn, "index.json", businesses: businesses)
  end

  def create(conn, %{"business" => business_params}) do
    with {:ok, %Business{} = business} <- DexContext.create_business(business_params) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    business = DexContext.get_business!(id)
  ...
  def update(conn, %{"id" => id, "business" => business_params}) do
    ...
    with {:ok, %Business{} = business} <- DexContext.update_business(business, business_params) do
      render(conn, "show.json", business: business)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %Business{}} <- DexContext.delete_business(business) do
